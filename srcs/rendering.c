...
/*   Updated: 2023/02/21 14:21:24 by junglee          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	render_background(t_all *all)
{
	...
	{
		mlx_destroy_image(all->mlx_ptr, all->exit.img);
...
static void	render_info(t_all *all)
{
	...
	mlx_string_put(all->mlx_ptr, all->win_ptr, 150, 80, \
	create_trgb(0, 0, 255, 0), walk_cnt);
	ft_printf("%d\n", all->walk_cnt);
	free(walk_cnt);
}
